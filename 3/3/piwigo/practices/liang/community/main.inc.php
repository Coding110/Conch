...
//include_once('/var/www/html/piwigo/include/functions_html.inc.php');
...
SELECT
  ...
  WHERE '. implode(' AND ', $where).' AND community_user = '.$user['id']
   ;
...
function delete_picture_end()
{
    ...
    if (isset($page['image_id']) and isset($page['category']['id']))
    {
...
function add_sendResponse($encodedResponse){
    global $conf, $user;
...
function add_register_user($user)
{
  ...
  $output =  create_virtual_category($user['username'], $conf['community']['user_albums_parent']);

  if (isset($output['error']))
  {
    return new PwgError(500, $output['error']);
  }
